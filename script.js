document.addEventListener('DOMContentLoaded', () => {
  ...
  // 7. ENVÍO Y VALIDACIÓN DEL FORMULARIO DE CONTACTO
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      if (submitTime - pageLoadTime < 1500) {
        formAlert.classList.add('error');
        formAlert.innerText = 'Detección de seguridad: Envío demasiado rápido.';
        return;
      }

      if (submitTime - lastSubmitTime < 30000 && lastSubmitTime !== 0) {
        formAlert.classList.add('error');
        formAlert.innerText = 'Por favor esperá 30 segundos antes de enviar otro mensaje.';
        return;
      }
      ...
      if (isValid) {
        ...
        submitBtn.querySelector('.btn-text').innerText = 'Enviando consulta...';

        const cleanData = {
          name: sanitizeInput(fullname.value.trim()),
          company: sanitizeInput(company.value.trim()),
          email: sanitizeInput(email.value.trim()),
          service: serviceType.value,
          users: userSlider ? userSlider.value : '10',
        ...
        try {
          ...
          if (response.ok) {
            ...
            formAlert.innerText = `¡Gracias ${cleanData.name}! Tu mensaje fue enviado con éxito.`;
            form.reset();
          ...
          formAlert.classList.add('success');
          formAlert.innerText = `¡Gracias ${cleanData.name}! Tu consulta fue enviada con éxito.`;
          form.reset();
          if (userCountDisplay) userCountDisplay.innerText = '10 usuarios';
        } finally {
